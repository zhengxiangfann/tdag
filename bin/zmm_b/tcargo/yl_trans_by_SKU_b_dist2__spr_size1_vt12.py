...
class Yltransbyskubdist2sprsize1vt12(BaseDag):
    '''
    ...
    def run_command(self):
        self.sql = """DROP  TABLE belle_sh.yl_trans_by_SKU_b_dist2_{invsunday}_spr_size1_vt12;
CREATE TABLE belle_sh.yl_trans_by_SKU_b_dist2_{invsunday}_spr_size1_vt12 AS
SELECT
*
,(CASE
  WHEN ind_clear=1
  ...
  WHEN sum_out_size1_clear<=cnt_size1_require_score AND cnt_size1_require_score<=cum_out_extra_size1_available AND inv_extra_size1_store<=0
  THEN if(ind_priority_inv>0 OR sum_qty_1week_store>0,greatest(inv_priority_size1,inv_size1_from_score_in),inv_end_size1_store)
  WHEN cnt_size1_require_score>cum_out_extra_size1_available AND sum_qty_1week_store<=0 AND ind_priority_inv<=0
  THEN if(ind_no_out=1,inv_end_size1_store,if(cum_out_size1_lag1>=cnt_size1_require_score-cum_out_extra_size1_available,least(1,inv_end_size1_store),0))
...
FROM belle_sh.yl_trans_by_SKU_b_dist1_{invsunday}_spr_size1_vt12
;""".format(**self.params)
        self.call()

if __name__ == '__main__':
    Yltransbyskubdist2sprsize1vt12().run_command()
