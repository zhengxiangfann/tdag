...
class Yltransbyskumastersprtestvt12(BaseDag):
    '''
    ...
    def run_command(self):
        self.sql = """DROP  TABLE belle_sh.yl_trans_by_SKU_master{invsunday}_spr_test_vt12;
SET hive.support.quoted.identifiers=NONE;
CREATE TABLE belle_sh.yl_trans_by_SKU_master{invsunday}_spr_test_vt12 AS
SELECT
...
FROM belle_sh.yl_trans_by_SKU_sale_inv_{invsunday}_spr a
LEFT JOIN belle_sh.yl_trans_by_SKU_days_nosale_{invsunday}_spr b
ON a.product_code=b.product_code AND a.store_no=b.store_no
LEFT JOIN belle_sh.yl_trans_by_SKU_store_no_out{invsunday}_spr c
ON a.store_no=c.store_no
LEFT JOIN belle_sh.yl_trans_by_SKU_score_{invsunday}_spr d
ON a.product_code=d.product_code AND a.store_no=d.store_no
LEFT JOIN (SELECT DISTINCT product_code,use_30D_ind_sku FROM belle_sh.yl_trans_by_SKU_score_{invsunday}_spr) d2
ON a.product_code=d2.product_code
LEFT JOIN belle_sh.yl_trans_by_SKU_inv_total_{invsunday}_spr e
ON a.product_code=e.product_code
LEFT JOIN belle_sh.yl_trans_by_SKU_storelevel f
ON a.store_no=f.store_no
LEFT JOIN belle_sh.yl_trans_by_SKU_events_{invsunday}_spr g
ON a.store_no=g.store_no
FULL OUTER JOIN belle_sh.yl_trans_by_SKU_priority_inv_{invsunday}_spr h
ON a.store_no=h.store_no AND a.product_code=h.product_code
LEFT JOIN belle_sh.yl_trans_by_SKU_clear_inv_{invsunday}_spr i
...
LEFT JOIN belle_sh.yl_trans_by_SKU_store_no_in{invsunday}_spr k
...
if __name__ == '__main__':
    Yltransbyskumastersprtestvt12().run_command()
