...
class Yltransbyskubsprpreparevt12(BaseDag):
    '''
    ...
    def run_command(self):
        self.sql = """DROP  TABLE belle_sh.yl_trans_by_SKU_b_spr_prepare_{invsunday}_vt12;
CREATE TABLE belle_sh.yl_trans_by_SKU_b_spr_prepare_{invsunday}_vt12 AS
SELECT
*
,sum(if((sum_qty_1week_store>0 OR inv_priority_225>0) AND ind_no_in<>1, greatest(greatest(inv_priority_225,if(inv_end_225_store=0 AND sum_qty_1week_store=1 AND sum_qty_2week_store=1 AND cnt_main_size>=3 AND (sku_qty_per_inv>=0.1 OR inv_end_220_store+inv_end_245_store>0),0,inv_225_from_score))-inv_end_225_store,0),0)) over(PARTITION BY product_code) AS cnt_225_require_score
,sum(if((sum_qty_1week_store>0 OR inv_priority_230>0) AND ind_no_in<>1, greatest(greatest(inv_priority_230,if(inv_end_230_store=0 AND sum_qty_1week_store=1 AND sum_qty_2week_store=1 AND cnt_main_size>=3 AND (sku_qty_per_inv>=0.1 OR inv_end_220_store+inv_end_245_store>0),0,inv_230_from_score))-inv_end_230_store,0),0)) over(PARTITION BY product_code) AS cnt_230_require_score
,sum(if((sum_qty_1week_store>0 OR inv_priority_235>0) AND ind_no_in<>1, greatest(greatest(inv_priority_235,if(inv_end_235_store=0 AND sum_qty_1week_store=1 AND sum_qty_2week_store=1 AND cnt_main_size>=3 AND (sku_qty_per_inv>=0.1 OR inv_end_220_store+inv_end_245_store>0),0,inv_235_from_score))-inv_end_235_store,0),0)) over(PARTITION BY product_code) AS cnt_235_require_score
,sum(if((sum_qty_1week_store>0 OR inv_priority_240>0) AND ind_no_in<>1, greatest(greatest(inv_priority_240,if(inv_end_240_store=0 AND sum_qty_1week_store=1 AND sum_qty_2week_store=1 AND cnt_main_size>=3 AND (sku_qty_per_inv>=0.1 OR inv_end_220_store+inv_end_245_store>0),0,inv_240_from_score))-inv_end_240_store,0),0)) over(PARTITION BY product_code) AS cnt_240_require_score
...
if __name__ == '__main__':
    Yltransbyskubsprpreparevt12().run_command()
