...
class Xwtransbyskubsprprepare20190421v11a(BaseDag):
    '''
    ...
    def run_command(self):
        self.sql = """DROP  TABLE belle_sh.xw_trans_by_SKU_b_spr_prepare_20190421_v11a;
CREATE TABLE belle_sh.xw_trans_by_SKU_b_spr_prepare_20190421_v11a AS
SELECT
*
,sum(if(sum_qty_1week_store>0 OR inv_priority_220>0 , greatest(greatest(inv_priority_220,inv_220_from_score)-inv_end_220_store,0),0)) over(PARTITION BY product_code) 																			 AS cnt_220_require_score
,sum(if(sum_qty_1week_store>0 OR inv_priority_225>0 OR (inv_end_225_store < 1 AND cnt_main_size <= 2 AND inv_end_store > 0), if(cnt_main_size_gt2 <= 2 AND inv_end_225_store <=0 , greatest(greatest(inv_priority_225,inv_225_from_score,inv_225_from_score_main)-inv_end_225_store,0), greatest(least(1,inv_225_from_score) - inv_end_225_store,0)),0)) over(PARTITION BY product_code) AS cnt_225_require_score
,sum(if(sum_qty_1week_store>0 OR inv_priority_230>0 OR (inv_end_230_store < 1 AND cnt_main_size <= 2 AND inv_end_store > 0), if(cnt_main_size_gt2 <= 2 AND inv_end_230_store <=0 , greatest(greatest(inv_priority_230,inv_230_from_score,inv_230_from_score_main)-inv_end_230_store,0), greatest(least(1,inv_230_from_score) - inv_end_230_store,0)),0)) over(PARTITION BY product_code) AS cnt_230_require_score
,sum(if(sum_qty_1week_store>0 OR inv_priority_235>0 OR (inv_end_235_store < 1 AND cnt_main_size <= 2 AND inv_end_store > 0), if(cnt_main_size_gt2 <= 2 AND inv_end_235_store <=0 , greatest(greatest(inv_priority_235,inv_235_from_score,inv_235_from_score_main)-inv_end_235_store,0), greatest(least(1,inv_235_from_score) - inv_end_235_store,0)),0)) over(PARTITION BY product_code) AS cnt_235_require_score
,sum(if(sum_qty_1week_store>0 OR inv_priority_240>0 OR (inv_end_240_store < 1 AND cnt_main_size <= 2 AND inv_end_store > 0), if(cnt_main_size_gt2 <= 2 AND inv_end_240_store <=0 , greatest(greatest(inv_priority_240,inv_240_from_score,inv_240_from_score_main)-inv_end_240_store,0), greatest(least(1,inv_240_from_score) - inv_end_240_store,0)),0)) over(PARTITION BY product_code) AS cnt_240_require_score
...
FROM
(SELECT *
	,if(label_sku_logic IN (2,3) AND (inv_end_225_store < 1 AND least(1,inv_end_225_store)+least(1,inv_end_230_store)+least(1,inv_end_235_store)+least(1,inv_end_240_store) <= 2),greatest(inv_225_from_score,1),inv_225_from_score) AS inv_225_from_score_main
	,if(label_sku_logic IN (2,3) AND (inv_end_230_store < 1 AND least(1,inv_end_225_store)+least(1,inv_end_230_store)+least(1,inv_end_235_store)+least(1,inv_end_240_store) <= 2),greatest(inv_225_from_score,1),inv_230_from_score) AS inv_230_from_score_main
	,if(label_sku_logic IN (2,3) AND (inv_end_235_store < 1 AND least(1,inv_end_225_store)+least(1,inv_end_230_store)+least(1,inv_end_235_store)+least(1,inv_end_240_store) <= 2),greatest(inv_225_from_score,1),inv_235_from_score) AS inv_235_from_score_main
	,if(label_sku_logic IN (2,3) AND (inv_end_240_store < 1 AND least(1,inv_end_225_store)+least(1,inv_end_230_store)+least(1,inv_end_235_store)+least(1,inv_end_240_store) <= 2),greatest(inv_225_from_score,1),inv_240_from_score) AS inv_240_from_score_main
...
if __name__ == '__main__':
    Xwtransbyskubsprprepare20190421v11a().run_command()
