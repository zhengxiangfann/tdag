...
FROM(
    ...
    FROM(
        ...
        FROM(
            SELECT *,
            LAG(cnt_qty_store, 1,NULL) OVER (PARTITION BY store_no ORDER BY weekYearSale ASC) AS cnt_qty_store_L1,
            LAG(cnt_qty_store, 2,NULL) OVER (PARTITION BY store_no ORDER BY weekYearSale ASC) AS cnt_qty_store_L2,
            LAG(cnt_qty_store, 3,NULL) OVER (PARTITION BY store_no ORDER BY weekYearSale ASC) AS cnt_qty_store_L3,
            lead(cnt_qty_store, 1,NULL) OVER (PARTITION BY store_no ORDER BY weekYearSale ASC) AS cnt_qty_store_F1,
            lead(cnt_qty_store, 2,NULL) OVER (PARTITION BY store_no ORDER BY weekYearSale ASC) AS cnt_qty_store_F2
            FROM(
                SELECT store_no, weekYearSale, WEEK,sale_year,
                sum(qty) AS cnt_qty_store,
                sum(sum_amt) AS sum_amt
                FROM belle_sh.mz_sku_store_size_week
                GROUP BY store_no, weekYearSale, WEEK,sale_year
                ) store_1
            ) store_2
        ) store_3
    ) store_4"""
