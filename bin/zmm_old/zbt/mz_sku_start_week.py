...
SELECT a.*,
       datediff(to_date(from_unixtime(UNIX_TIMESTAMP(sunday_date,'yyyyMMdd'))),to_date(from_unixtime(UNIX_TIMESTAMP(start_date,'yyyyMMdd')))) AS online_days
FROM
	(SELECT product_code, seasonYearSKU,
	...
	GROUP BY product_code, seasonYearSKU
	) a"""
        self.call()
