...
class Xwtransbyskubsprprepare2size4vt12(BaseDag):
    '''
    ...
    def run_command(self):
        self.sql = """DROP  TABLE belle_sh.xw_trans_by_SKU_b_spr_prepare2_{invsunday}_size4_vt12;
CREATE TABLE belle_sh.xw_trans_by_SKU_b_spr_prepare2_{invsunday}_size4_vt12 AS
SELECT
*
,if(((sum_qty_1week_store>0 AND cnt_main_size4+least(1,inv_end_245_store)<=2) OR inv_priority_220>0) AND ind_no_in<>1,inv_220_from_score,inv_end_220_store) AS inv_220_from_score_v2
,sum(if(
        ((sum_qty_1week_store>0 AND cnt_main_size4+least(1,inv_end_245_store)<=2) OR inv_priority_220>0) AND ind_no_in<>1
       , greatest(greatest(inv_priority_220,inv_220_from_score)-inv_end_220_store,0)
	   ,0
	   )
...
FROM belle_sh.xw_trans_by_SKU_b_spr_prepare_{invsunday}_size4_vt12 a
;""".format(**self.params)
        self.call()

if __name__ == '__main__':
    Xwtransbyskubsprprepare2size4vt12().run_command()
