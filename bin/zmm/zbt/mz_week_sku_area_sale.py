  ...
  (SELECT *,
   ...
   FROM
     (SELECT *,
      ...
      FROM
        (SELECT *,
                LAG(cnt_qty_sku_area, 1, NULL) OVER (PARTITION BY product_code,
                                                                  seasonYearSKU
                                                     ORDER BY weekYearSale ASC) AS cnt_qty_sku_area_L1,
                                                    LAG(cnt_qty_sku_area, 2, NULL) OVER (PARTITION BY product_code,
                                                                                                      seasonYearSKU
                                                                                         ORDER BY weekYearSale ASC) AS cnt_qty_sku_area_L2,
                                                                                        LAG(cnt_qty_sku_area, 3, NULL) OVER (PARTITION BY product_code,
                                                                                                                                          seasonYearSKU
                                                                                                                             ORDER BY weekYearSale ASC) AS cnt_qty_sku_area_L3,
                                                                                                                            lead(cnt_qty_sku_area, 1, NULL) OVER (PARTITION BY product_code,
                                                                                                                                                                               seasonYearSKU
                                                                                                                                                                  ORDER BY weekYearSale ASC) AS cnt_qty_sku_area_F1,
                                                                                                                                                                 lead(cnt_qty_sku_area, 2, NULL) OVER (PARTITION BY product_code,
                                                                                                                                                                                                                    seasonYearSKU
                                                                                                                                                                                                       ORDER BY weekYearSale ASC) AS cnt_qty_sku_area_F2
         FROM
           (SELECT weekYearSale,
                   product_code,
                   seasonYearSKU,
            ...
            GROUP BY weekYearSale,
                     product_code,
                     seasonYearSKU) sku_area_1) sku_area_2) sku_area_3) sku_area_4"""
        self.call()
