  ...
  (SELECT *,
   ...
   FROM
     (SELECT *,
      ...
      FROM
        (SELECT *,
                LAG(cnt_qty_season_area, 1, NULL) OVER (PARTITION BY seasonYearSKU
                                                        ORDER BY weekYearSale ASC) AS cnt_qty_season_area_L1,
                                                       LAG(cnt_qty_season_area, 2, NULL) OVER (PARTITION BY seasonYearSKU
                                                                                               ORDER BY weekYearSale ASC) AS cnt_qty_season_area_L2,
                                                                                              LAG(cnt_qty_season_area, 3, NULL) OVER (PARTITION BY seasonYearSKU
                                                                                                                                      ORDER BY weekYearSale ASC) AS cnt_qty_season_area_L3,
                                                                                                                                     lead(cnt_qty_season_area, 1, NULL) OVER (PARTITION BY seasonYearSKU
                                                                                                                                                                              ORDER BY weekYearSale ASC) AS cnt_qty_season_area_F1,
                                                                                                                                                                             lead(cnt_qty_season_area, 2, NULL) OVER (PARTITION BY seasonYearSKU
                                                                                                                                                                                                                      ORDER BY weekYearSale ASC) AS cnt_qty_season_area_F2
         FROM
           (SELECT weekYearSale,
                   WEEK,
                   sale_year,
            ...
            GROUP BY weekYearSale,
                     WEEK,
                     sale_year,
                     seasonYearSKU,
                     product_season_name,
                     product_year_name) season_area_1) season_area_2) season_area_3) season_area_4"""
        self.call()
